...
def train_hadamard(num_iterations, config):
    ...
    print(f"\n{'=' * 50}")
    print(f"Training Hadamard Network with {num_iterations} iterations")
    ...
    for epoch in range(config['epochs']):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            for _ in range(num_iterations):
                ...
                iteration_loss = image_loss + config['lambda_weight'] * label_loss
                iteration_losses.append(iteration_loss)

            # Sum losses from all iterations
            total_loss = sum(iteration_losses)
            total_loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    for iterations, accuracy in results.items():
        print(f"Hadamard Sum Loss with {iterations} iterations: {accuracy:.2f}%")